...
const p = (f: string) => `/images/retail/${f}`
...
const STATS = [
  { num: '6+', label: 'Store Departments' },
  { num: '1', label: 'Floor Plan System' },
  { num: '3', label: 'Tools in Pipeline' },
]

const TOOLS = ['3ds Max', 'Photoshop', 'AutoCAD']
    ...
    <div style={{ backgroundColor: BG, minHeight: '100vh', paddingBottom: isMobile ? '140px' : '120px' }}>
      ...
      >
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '32px 16px' : '48px 36px', boxSizing: 'border-box' }}>
          <Img src={p('retail-hero.png')} alt="Retail store visualization" maxHeight={isMobile ? '280px' : '70vh'} />
        </div>
      </motion.div>
        ...
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '40px 16px' : '64px 36px', boxSizing: 'border-box', display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? 32 : 64, alignItems: 'flex-start' }}>
          <div style={{ width: isMobile ? '100%' : '45%', flexShrink: 0 }}>
            ...
            <p style={{ fontSize: isMobile ? 17 : 20, lineHeight: 1.85, color: MUTED }}>
              Signage design and environmental graphics system for a large-format retail store. Working from AutoCAD floor plans, the project covers department identity, wayfinding hierarchy, and large-format print applications. 3ds Max was used to visualize signage in context — including produce, deli, cereals, and restaurant zones — before any fabrication commitment.
            </p>
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <Img src={p('retail-cereals.png')} alt="Retail cereals aisle" maxHeight={isMobile ? '280px' : '480px'} />
          </div>
        </div>
      </motion.section>
      ...
      >
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '40px 16px' : '64px 36px', boxSizing: 'border-box' }}>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 16 : 24 }}>
            <Img src={p('retail-office.png')} alt="Retail office area" maxHeight={isMobile ? '280px' : '480px'} />
            <Img src={p('retail-floor6.png')} alt="Retail floor plan" maxHeight={isMobile ? '280px' : '480px'} />
          </div>
          <Caption text="Working from AutoCAD floor plans, each zone was assigned its own signage system before full 3D visualization — reducing costly redesigns after fabrication begins." />
        </div>
      </motion.section>
      ...
      >
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '0 16px' : '0', display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: 'stretch' }}>
          <div style={{ width: isMobile ? '100%' : '40%', flexShrink: 0, background: ACCENT, padding: isMobile ? '36px 16px' : '56px 36px', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: ACCENT_TEXT, opacity: 0.65, marginBottom: 20 }}>Scale</p>
            <h2 style={{ fontSize: isMobile ? 32 : 44, fontWeight: 700, color: ACCENT_TEXT, lineHeight: 1.2, margin: 0 }}>
              Six departments.<br />One visual<br />language.
            </h2>
          </div>
          <div style={{ flex: 1, minWidth: 0, background: CONTENT, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Img src={p('retail-produce.png')} alt="Retail produce section" maxHeight={isMobile ? '280px' : '480px'} />
          </div>
        </div>
      </motion.section>
      ...
      >
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '40px 16px' : '64px 36px', boxSizing: 'border-box' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 16 : 24 }}>
            <Img src={p('retail-deli.png')} alt="Retail deli section" maxHeight={isMobile ? '280px' : '480px'} />
            <Img src={p('retail-restaurant.png')} alt="Retail restaurant" maxHeight={isMobile ? '280px' : '480px'} />
            <Img src={p('retail-floorplan.png')} alt="Retail floor plan" maxHeight={isMobile ? '280px' : '480px'} />
            <Img src={p('retail-cereals.png')} alt="Retail cereals" maxHeight={isMobile ? '280px' : '480px'} />
          </div>
        </div>
      </motion.section>
