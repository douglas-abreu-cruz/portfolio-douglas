...
const p = (f: string) => `/images/attic/${f}`
...
const STATS = [
  { num: '3', label: 'Color Variants' },
  { num: '3', label: 'Functional Zones' },
  { num: '1', label: 'Structural Constraint Solved' },
]

const TOOLS = ['SketchUp', 'V-Ray']
    ...
    <div style={{ backgroundColor: BG, minHeight: '100vh', paddingBottom: isMobile ? '140px' : '120px' }}>
      ...
      >
        <div style={{ maxWidth: 1440, margin: '0 auto', width: '100%' }}>
          ...
          <h1 style={{ fontSize: isMobile ? 'clamp(52px, 16vw, 88px)' : 'clamp(64px, 8vw, 130px)', fontWeight: 700, letterSpacing: '-0.02em', lineHeight: 0.95, color: FG, marginBottom: 20 }}>
            Attic Loft<br />Bat Cave
          </h1>
          <p style={{ fontSize: isMobile ? 20 : 28, color: MUTED, fontWeight: 400, lineHeight: 1.5 }}>
            Turning a forgotten attic into a personal sanctuary
          </p>
        </div>
      </motion.section>
        ...
        <div style={{ maxWidth: 1440, margin: '0 auto', display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? 32 : 64, alignItems: 'flex-start' }}>
          <div style={{ width: isMobile ? '100%' : '45%', flexShrink: 0 }}>
            ...
            <p style={{ fontSize: isMobile ? 17 : 20, lineHeight: 1.85, color: MUTED }}>
              A common North American challenge — an underused attic with sloped ceilings and awkward proportions. The proposal transforms the raw space into a fully functional bat cave: home office, fitness area, and lounge zone combined. Modeled from real measurements in SketchUp and rendered in V-Ray to present the concept with photorealistic clarity.
            </p>
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <Img src={p('variant-green.png')} alt="Attic loft green variant" maxHeight={isMobile ? '280px' : '480px'} />
          </div>
        </div>
      </motion.section>
      ...
      >
        <div style={{ maxWidth: 1440, margin: '0 auto' }}>
          ...
          <Caption text="The sloped ceiling is the defining constraint — the design turns the limitation into identity, using the roofline geometry as a visual anchor." />
        </div>
      </motion.section>
      ...
      >
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '0 16px' : '0', display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: 'stretch' }}>
          ...
          <div style={{ flex: 1, minWidth: 0, background: CONTENT, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Img src={p('hero.png')} alt="Attic loft overview" maxHeight={isMobile ? '280px' : '480px'} />
          </div>
        </div>
      </motion.section>
      ...
      >
        <div style={{ maxWidth: 1440, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 16 : 24 }}>
            ...
            <Img src={p('variant-green.png')} alt="Green variant" maxHeight={isMobile ? '280px' : '480px'} />
            <Img src={p('variant-teal.png')} alt="Teal variant" maxHeight={isMobile ? '280px' : '480px'} />
            <Img src={p('variant-blue.png')} alt="Blue variant" maxHeight={isMobile ? '280px' : '480px'} />
          </div>
        </div>
      </motion.section>
