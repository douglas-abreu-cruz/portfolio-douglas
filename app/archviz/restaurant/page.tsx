...
const p = (f: string) => `/images/restaurant/${f}`
...
const STATS = [
  { num: '4', label: 'Camera Angles' },
  { num: '3', label: 'Lighting Scenarios' },
  { num: '1', label: 'Abandoned Floor Plan' },
]

const TOOLS = ['SketchUp', 'Twinmotion']
    ...
    <div style={{ backgroundColor: BG, minHeight: '100vh', paddingBottom: isMobile ? '140px' : '120px' }}>
      ...
      >
        <div style={{ maxWidth: 1440, margin: '0 auto', width: '100%' }}>
          ...
          <h1 style={{ fontSize: isMobile ? 'clamp(52px, 16vw, 88px)' : 'clamp(64px, 8vw, 130px)', fontWeight: 700, letterSpacing: '-0.02em', lineHeight: 0.95, color: FG, marginBottom: 20 }}>
            Restaurant<br />From Ruins
          </h1>
          <p style={{ fontSize: isMobile ? 20 : 28, color: MUTED, fontWeight: 400, lineHeight: 1.5 }}>
            An abandoned floor plan becomes a full-service restaurant — built entirely in Twinmotion.
          </p>
        </div>
      </motion.section>
      ...
      >
        <div style={{ maxWidth: 1440, margin: '0 auto' }}>
          <Img src={p('hero.png')} alt="Restaurant visualization hero" maxHeight={isMobile ? '280px' : '70vh'} />
        </div>
      </motion.div>
        ...
        <div style={{ maxWidth: 1440, margin: '0 auto', display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? 32 : 64, alignItems: 'flex-start' }}>
          <div style={{ width: isMobile ? '100%' : '45%', flexShrink: 0 }}>
            ...
            <p style={{ fontSize: isMobile ? 17 : 20, lineHeight: 1.85, color: MUTED }}>
              Starting only from a bare architectural floor plan of an abandoned commercial space, the brief was to design and fully visualize a functioning restaurant. The entire space was modeled in SketchUp and rendered in Twinmotion, exploring how the same environment reads across four camera angles and three distinct lighting scenarios — daytime, dusk, and night.
            </p>
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <Img src={p('angle-d-day.png')} alt="Restaurant angle D daytime" maxHeight={isMobile ? '280px' : '480px'} />
          </div>
        </div>
      </motion.section>
      ...
      >
        <div style={{ maxWidth: 1440, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 16 : 24 }}>
            <Img src={p('angle-a-dusk.png')} alt="Restaurant dusk lighting" maxHeight={isMobile ? '280px' : '480px'} />
            <Img src={p('angle-b-night.png')} alt="Restaurant night lighting" maxHeight={isMobile ? '280px' : '480px'} />
          </div>
          <Caption text="Dusk and night shots reveal how the interior lighting strategy performs — warm ambient tones balanced against cool exterior sky." />
        </div>
      </motion.section>
      ...
      >
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '0 16px' : '0', display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: 'stretch' }}>
          <div style={{ width: isMobile ? '100%' : '40%', flexShrink: 0, background: ACCENT, padding: isMobile ? '36px 24px' : '56px 48px', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: ACCENT_TEXT, opacity: 0.65, marginBottom: 20 }}>Process</p>
            <h2 style={{ fontSize: isMobile ? 32 : 44, fontWeight: 700, color: ACCENT_TEXT, lineHeight: 1.2, margin: 0 }}>
              From floor plan<br />to opening night —<br />in 3D.
            </h2>
          </div>
          <div style={{ flex: 1, minWidth: 0, background: CONTENT, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Img src={p('angle-c-warm.png')} alt="Restaurant warm interior" maxHeight={isMobile ? '280px' : '480px'} />
          </div>
        </div>
      </motion.section>
      ...
      >
        <div style={{ maxWidth: 1440, margin: '0 auto' }}>
          <SectionLabel text="Angles &amp; Lighting" />
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 16 : 24 }}>
            <Img src={p('angle-a-warm.png')} alt="Angle A warm" maxHeight={isMobile ? '280px' : '480px'} />
            <Img src={p('angle-b-dark.png')} alt="Angle B dark" maxHeight={isMobile ? '280px' : '480px'} />
            <Img src={p('angle-c-day.png')} alt="Angle C day" maxHeight={isMobile ? '280px' : '480px'} />
            <Img src={p('angle-d-day.png')} alt="Angle D day" maxHeight={isMobile ? '280px' : '480px'} />
          </div>
        </div>
      </motion.section>
