...
const p = (f: string) => `/images/bathroom/${f}`
...
const STATS = [
  { num: '2', label: 'Bathroom Projects' },
  { num: '9', label: 'Scenes Rendered' },
  { num: '4', label: 'Vanity Colorways' },
]
    ...
    <div style={{ backgroundColor: BG, minHeight: '100vh', paddingBottom: isMobile ? '140px' : '120px' }}>
      ...
      >
        <div style={{ maxWidth: 1440, margin: '0 auto', width: '100%', padding: isMobile ? '48px 16px 36px' : '72px 36px 36px', boxSizing: 'border-box' }}>
          ...
          <HeroTitle style={{ fontSize: isMobile ? 'clamp(52px, 16vw, 88px)' : 'clamp(64px, 8vw, 130px)', fontWeight: 700, letterSpacing: '-0.02em', lineHeight: 0.95, color: FG, marginBottom: 20 }}>
            Bathroom<br />Renovations
          </HeroTitle>
          <p style={{ fontSize: isMobile ? 20 : 28, color: MUTED, fontWeight: 400, lineHeight: 1.5 }}>
            Biophilic design meets vanity renovation — two projects, one discipline.
          </p>
        </div>
      </motion.section>
      ...
      >
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '32px 16px' : '48px 36px', boxSizing: 'border-box' }}>
          <Img src={p('brand-new-plants.png')} alt="Brand New bathroom with plants" maxHeight={isMobile ? '280px' : '70vh'} />
        </div>
      </motion.div>
        ...
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '40px 16px' : '64px 36px', boxSizing: 'border-box', display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? 32 : 64, alignItems: 'flex-start' }}>
          <div style={{ width: isMobile ? '100%' : '45%', flexShrink: 0 }}>
            ...
            <p style={{ fontSize: isMobile ? 17 : 20, lineHeight: 1.85, color: MUTED }}>
              Two distinct residential bathroom briefs visualized in SketchUp and Photoshop. The first — a brand-new biophilic bathroom — brings natural materials, slate textures, and plant life into a cohesive design. The second — a vanity renovation — explores four different color and finish combinations to help the client choose before committing.
            </p>
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <Img src={p('brand-new-vanity.png')} alt="Brand New bathroom vanity" maxHeight={isMobile ? '280px' : '480px'} />
          </div>
        </div>
      </motion.section>
      ...
      >
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '40px 16px' : '64px 36px', boxSizing: 'border-box' }}>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 16 : 24 }}>
            <Img src={p('brand-new-process.png')} alt="Brand New process view" maxHeight={isMobile ? '280px' : '480px'} />
            <Img src={p('brand-new-detail.png')} alt="Brand New detail view" maxHeight={isMobile ? '280px' : '480px'} />
          </div>
          <Caption text="The biophilic brief required material research: real slate textures, aged brass fixtures, and plant species that survive a low-light bathroom environment." />
        </div>
      </motion.section>
      ...
      >
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '0 16px' : '0', display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: 'stretch' }}>
          <div style={{ width: isMobile ? '100%' : '40%', flexShrink: 0, background: ACCENT, padding: isMobile ? '36px 16px' : '56px 36px', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: ACCENT_TEXT, opacity: 0.65, marginBottom: 20 }}>Vanity Series</p>
            <h2 style={{ fontSize: isMobile ? 32 : 44, fontWeight: 700, color: ACCENT_TEXT, lineHeight: 1.2, margin: 0 }}>
              4 colorways.<br />One decision<br />made easy.
            </h2>
          </div>
          <div style={{ flex: 1, minWidth: 0, background: CONTENT, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Img src={p('brand-new-slats.png')} alt="Brand New bathroom slats" maxHeight={isMobile ? '280px' : '480px'} />
          </div>
        </div>
      </motion.section>

      {/* 5. DETAIL GRID — Vanity Colorways */}
      <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={VP} transition={FV}
        style={{ background: CONTENT }}
      >
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '40px 16px' : '64px 36px', boxSizing: 'border-box' }}>
          <SectionLabel text="Vanity Renovation — All Colorways" />
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 16 : 24 }}>
            <Img src={p('vanity-gray-mirror1.png')} alt="Vanity gray mirror" maxHeight={isMobile ? '280px' : '480px'} />
            <Img src={p('vanity-navy-white.png')} alt="Vanity navy white" maxHeight={isMobile ? '280px' : '480px'} />
            <Img src={p('vanity-navy-gray.png')} alt="Vanity navy gray" maxHeight={isMobile ? '280px' : '480px'} />
            <Img src={p('vanity-gray-arch.png')} alt="Vanity gray arch" maxHeight={isMobile ? '280px' : '480px'} />
          </div>
        </div>
      </motion.section>
