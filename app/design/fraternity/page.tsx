...
const p = (f: string) => `/images/fraternity/${f}`
...
const STATS = [
  { num: '7+',   label: 'Campaigns designed' },
  { num: '2',    label: 'Media types' },
  { num: '100%', label: 'Volunteer driven' },
]

const TOOLS = ['Illustrator', 'Photoshop', 'Canva']
    ...
    <div style={{ backgroundColor: BG, minHeight: '100vh', paddingBottom: isMobile ? '160px' : '140px' }}>
      ...
      <motion.section initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.7 }}
        ...
        <div style={{ maxWidth: 1440, margin: '0 auto', width: '100%' }}>
          ...
          <h1 style={{ fontSize: isMobile ? 'clamp(88px, 28vw, 144px)' : 'clamp(88px, 14vw, 192px)', fontWeight: 700, letterSpacing: '-0.02em', lineHeight: 0.95, color: '#ffffff', marginBottom: 20 }}>
        ...
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '40px 16px' : '64px 36px 40px', display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? 32 : 0, alignItems: 'flex-start', overflow: 'visible' }}>
          <div style={{ width: isMobile ? '100%' : '60%', flexShrink: 0, paddingRight: isMobile ? 0 : 56, boxSizing: 'border-box' }}>
            ...
            <p style={{ fontSize: 28, lineHeight: 1.8, color: '#444', maxWidth: 520 }}>
              Graphic design support for a non-profit volunteer organization promoting solidarity and fraternity. Social media content and printed materials for campaigns and events.
            </p>
          </div>
          <div style={{ width: isMobile ? '100%' : '40%', marginTop: isMobile ? 0 : -100, position: 'relative', zIndex: 2 }}>
            <Img src={p('design-fraternity-hero.png')} alt="Fraternity Without Borders" maxHeight="60vh" imgStyle={{ boxShadow: '0 16px 48px rgba(0,0,0,0.2)' }} />
          </div>
        </div>
      </motion.section>
      ...
      <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={VP} transition={FV}
        ...
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '0 16px 40px' : '0 36px 64px', display: 'flex', flexDirection: 'column', gap: 32 }}>
          <Img src={p('design-fraternity-card.png')} alt="Campaign card" maxHeight="480px" wrapStyle={{ height: '480px', width: '100%' }} imgStyle={{ height: '100%' }} />
          <Img src={p('design-fraternity-families.png')} alt="Families campaign" maxHeight="480px" wrapStyle={{ height: '480px', width: '100%' }} imgStyle={{ height: '100%' }} />
        </div>
      </motion.section>
        ...
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '0 16px' : '0 36px' }}>
          ...
          {isMobile ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              ...
              <Img src={p('design-fraternity-hero.png')} alt="Fraternity Without Borders" maxHeight="380px" wrapStyle={{ height: '380px' }} imgStyle={{ height: '100%' }} />
              <p style={{ fontSize: 28, lineHeight: 1.7, color: MUTED, margin: 0 }}>
                Every piece balanced sensitivity with the ability to drive real engagement. Social media content and printed materials carried the organization mission forward.
              </p>
            </div>
          ) : (
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 32 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <Img src={p('design-fraternity-hero.png')} alt="Fraternity Without Borders" maxHeight="480px" wrapStyle={{ height: '480px' }} imgStyle={{ height: '100%', boxShadow: '0 10px 36px rgba(0,0,0,0.22)' }} />
              </div>
            </div>
          )}
        </div>
      ...
      <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={VP} transition={FV}
        ...
        <div style={{ maxWidth: 1440, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 24 : 32, marginBottom: isMobile ? 24 : 32 }}>
            <div>
              <Img src={p('design-fraternity-nonprofit.png')} alt="Nonprofit Day" maxHeight="480px" wrapStyle={{ height: '480px' }} imgStyle={{ height: '100%' }} />
              <Caption text="Nonprofit Day" />
            </div>
            <div>
              <Img src={p('design-fraternity-fathersday.png')} alt="Fathers Day" maxHeight="480px" wrapStyle={{ height: '480px' }} imgStyle={{ height: '100%' }} />
              <Caption text="Fathers Day" />
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 24 : 32 }}>
            <div>
              <Img src={p('design-fraternity-christmas.png')} alt="Christmas campaign" maxHeight="480px" wrapStyle={{ height: '480px' }} imgStyle={{ height: '100%' }} />
              <Caption text="Christmas campaign" />
            </div>
            <div>
              <Img src={p('design-fraternity-mission.png')} alt="Mission piece" maxHeight="480px" wrapStyle={{ height: '480px' }} imgStyle={{ height: '100%' }} />
              <Caption text="Mission piece" />
            </div>
          </div>
        </div>
      </motion.section>
      ...
      <footer style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 50, background: '#fafafa', borderTop: '0.5px solid #e8e8e8' }}>
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '24px 16px' : '28px 36px', display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: isMobile ? 'flex-start' : 'center', gap: isMobile ? 20 : 0 }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {TOOLS.map(tool => (
              <span key={tool} style={{ fontSize: 22, letterSpacing: '0.08em', padding: '4px 12px', border: '1px solid rgba(48,92,222,0.3)', color: BG }}>
                {tool}
              </span>
            ))}
          </div>
          <button onClick={() => router.push('/design')} style={{ background: 'none', border: 'none', padding: 0, color: BG, cursor: 'pointer', fontSize: 26, letterSpacing: '0.04em' }}>
