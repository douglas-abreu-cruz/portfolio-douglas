...
const p = (f: string) => `/images/uaipizza/${f}`

const EYEBROW: CSSProperties = {
  fontSize: 22, fontWeight: 600, letterSpacing: '0.1em',
  textTransform: 'uppercase', color: '#1a1008', marginBottom: 20,
}
...
function Caption({ text }: { text: string }) {
  return (
    <p style={{ marginTop: 8, fontSize: 22, letterSpacing: '0.06em', color: '#1a1008', textAlign: 'center' }}>{text}</p>
  )
}

const STATS = [
  { num: '1',    label: 'Full brand manual' },
  { num: '3',    label: 'Digital platforms' },
  { num: '100%', label: 'Print ready' },
]

const TOOLS = ['Illustrator', 'Figma', 'Canva']

export default function UaiPizzaPage() {
  ...
  return (
    <div style={{ backgroundColor: BG, minHeight: '100vh', paddingBottom: isMobile ? '160px' : '140px' }}>
      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 50, backgroundColor: BG, borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '14px 16px' : '14px 36px', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexShrink: 0 }}>
            <button onClick={() => router.push('/design')} style={{ background: 'none', border: '1px solid rgba(242,184,37,0.3)', color: MUTED, cursor: 'pointer', padding: '5px 12px', fontSize: 13, letterSpacing: '0.06em', whiteSpace: 'nowrap', borderRadius: 0, fontFamily: 'inherit', fontWeight: 600 }}>
              ← Graphic Design
            </button>
            <button onClick={() => router.push('/')} style={{ background: 'none', border: '1px solid rgba(242,184,37,0.3)', color: MUTED, cursor: 'pointer', padding: '5px 12px', fontSize: 13, letterSpacing: '0.06em', whiteSpace: 'nowrap', borderRadius: 0, fontFamily: 'inherit', fontWeight: 600 }}>
              ← Home
            </button>
          </div>
          {!isMobile && (
      ...
      <motion.section initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.7 }}
        style={{ background: `linear-gradient(140deg, ${BG} 0%, #8B1A1A 100%)`, minHeight: 320, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: 0 }}>
        <div style={{ maxWidth: 1440, margin: '0 auto', width: '100%', padding: isMobile ? '48px 16px 36px' : '72px 36px 36px', boxSizing: 'border-box' }}>
          <div style={{ display: 'flex', gap: 6, marginBottom: 24 }}>
            <div style={{ width: 32, height: 4, backgroundColor: '#C0392B' }} />
            <div style={{ width: 20, height: 4, backgroundColor: ACCENT }} />
          </div>
        ...
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '40px 16px' : '64px 36px 40px', display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? 32 : 0, alignItems: 'flex-start', overflow: 'visible' }}>
          <div style={{ width: isMobile ? '100%' : '60%', flexShrink: 0, paddingRight: isMobile ? 0 : 56, boxSizing: 'border-box' }}>
            ...
            <p style={{ fontSize: 28, lineHeight: 1.8, color: '#444', maxWidth: 520 }}>
              A rebrand for a Brazilian pizzeria in Toronto, built to support marketing, boost social media engagement and increase sales. Full brand manual with color palette and design patterns.
            </p>
          </div>
          <div style={{ width: isMobile ? '100%' : '40%', marginTop: isMobile ? 0 : -100, position: 'relative', zIndex: 2 }}>
            <Img src={p('design-uaipizza-hero.png')} alt="Uai Pizza" maxHeight="60vh" imgStyle={{ boxShadow: '0 16px 48px rgba(0,0,0,0.2)' }} />
          </div>
        </div>
      </motion.section>
      ...
      <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={VP} transition={FV}
        ...
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '0 16px 40px' : '0 36px 64px', display: 'flex', flexDirection: 'column', gap: 32 }}>
          <div>
            <Img src={p('design-uaipizza-comparison.png')} alt="Before and after" maxHeight="480px" wrapStyle={{ height: '480px', width: '100%' }} imgStyle={{ height: '100%' }} />
            <Caption text="Before & after — logo evolution" />
          </div>
          <div>
            <Img src={p('design-uaipizza-concept.png')} alt="Concept development" maxHeight="480px" wrapStyle={{ height: '480px', width: '100%' }} imgStyle={{ height: '100%' }} />
            <Caption text="Conceptual systematization" />
          </div>
        </div>
      </motion.section>
        ...
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '0 16px' : '0 36px' }}>
          ...
          {isMobile ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              ...
              <Img src={p('design-uaipizza-positioning.png')} alt="Brand positioning" maxHeight="380px" wrapStyle={{ height: '380px' }} imgStyle={{ height: '100%' }} />
              <p style={{ fontSize: 28, lineHeight: 1.7, color: MUTED, margin: 0 }}>
                The rebrand opened opportunities across digital and print. A complete brand manual created consistency for future applications.
              </p>
            </div>
          ) : (
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 32 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <Img src={p('design-uaipizza-positioning.png')} alt="Brand positioning" maxHeight="480px" wrapStyle={{ height: '480px' }} imgStyle={{ height: '100%', boxShadow: '0 10px 36px rgba(0,0,0,0.22)' }} />
              </div>
            </div>
          )}
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)', gap: 16 }}>
            {STATS.map(card => (
              <div key={card.num} style={{ background: '#f0f0ee', padding: 24 }}>
                <p style={{ fontSize: 44, fontWeight: 500, color: '#8B1A1A', margin: '0 0 6px' }}>{card.num}</p>
      ...
      <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={VP} transition={FV}
        ...
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '32px 16px 160px' : '48px 36px 140px', boxSizing: 'border-box' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 24 : 32, marginBottom: isMobile ? 24 : 32 }}>
            <div>
              <Img src={p('design-uaipizza-palette.png')} alt="Color palette" maxHeight="480px" wrapStyle={{ height: '480px' }} imgStyle={{ height: '100%' }} />
              <Caption text="Color palette & textures" />
            </div>
            <div>
              <Img src={p('design-uaipizza-box.png')} alt="Packaging" maxHeight="480px" wrapStyle={{ height: '480px' }} imgStyle={{ height: '100%' }} />
              <Caption text="Pizza box packaging" />
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 24 : 32 }}>
            <div>
              <Img src={p('design-uaipizza-mockups.png')} alt="Brand mockups" maxHeight="480px" wrapStyle={{ height: '480px' }} imgStyle={{ height: '100%' }} />
              <Caption text="Brand applications" />
            </div>
            <div>
              <Img src={p('design-uaipizza-website.png')} alt="Website design" maxHeight="480px" wrapStyle={{ height: '480px' }} imgStyle={{ height: '100%' }} />
              <Caption text="Website design" />
            </div>
          </div>
        </div>
      </motion.section>
      ...
      <footer style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 50, background: '#fafafa', borderTop: '0.5px solid #e8e8e8' }}>
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '16px' : '28px 36px', display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: isMobile ? 'flex-start' : 'center', gap: isMobile ? 20 : 0 }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {TOOLS.map(tool => (
              <span key={tool} style={{ fontSize: isMobile ? 13 : 22, letterSpacing: '0.08em', padding: '4px 12px', border: '1px solid rgba(192,57,43,0.4)', color: BG }}>
                {tool}
              </span>
            ))}
          </div>
