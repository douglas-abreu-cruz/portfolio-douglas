...
const v = (f: string) => `/images/vaction/${f}`
  ...
  return (
    <div style={{ backgroundColor: BG, minHeight: '100vh', paddingBottom: isMobile ? '160px' : '140px' }}>
      ...
      <header style={{
        ...
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '14px 16px' : '14px 36px', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexShrink: 0 }}>
            <button onClick={() => router.push('/design')} style={{ background: 'none', border: '1px solid rgba(105,180,232,0.3)', color: MUTED, cursor: 'pointer', padding: '5px 12px', fontSize: 13, letterSpacing: '0.06em', whiteSpace: 'nowrap', borderRadius: 0, fontFamily: 'inherit', fontWeight: 600 }}>
              ← Graphic Design
            </button>
            <button onClick={() => router.push('/')} style={{ background: 'none', border: '1px solid rgba(105,180,232,0.3)', color: MUTED, cursor: 'pointer', padding: '5px 12px', fontSize: 13, letterSpacing: '0.06em', whiteSpace: 'nowrap', borderRadius: 0, fontFamily: 'inherit', fontWeight: 600 }}>
              ← Home
            </button>
          </div>
          {!isMobile && (
      ...
      <motion.section
        ...
        style={{
          background: `linear-gradient(140deg, ${BG} 0%, #1a3a8a 100%)`,
          minHeight: 320,
          display: 'flex',
      ...
      >
        <div style={{ maxWidth: 1440, margin: '0 auto', width: '100%', padding: isMobile ? '48px 16px 36px' : '72px 36px 36px', boxSizing: 'border-box' }}>
          <div style={{ display: 'flex', gap: 6, marginBottom: 24 }}>
            <div style={{ width: 32, height: 4, backgroundColor: ORANGE }} />
            <div style={{ width: 20, height: 4, backgroundColor: YELLOW }} />
          </div>
          <p style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ fontSize: 28, lineHeight: 1.8, color: '#444', maxWidth: 520 }}>
              V.Action needed a complete visual identity from scratch — a brand
              that could capture the energy of Canadian winter sports and adventure
              tourism, while feeling modern, bold and internationally competitive.
            </p>
          </div>
          <div style={{
            ...
            <Img
              src={v('design-vaction-hero.png')}
              alt="V.Action brand hero"
              maxHeight="60vh"
      ...
      >
        <div style={{
          ...
          <div>
            <Img
              src={v('design-vaction-logo.png')}
              alt="V.Action logo system"
              maxHeight="480px"
              wrapStyle={{ height: '480px', width: '100%' }}
              imgStyle={{ height: '100%' }}
            />
            <Caption text="Logo system — diagonal split identity" />
          </div>
          <div>
            <Img
              src={v('design-vaction-color-palette.png')}
              alt="V.Action color palette"
              maxHeight="480px"
              wrapStyle={{ height: '480px', width: '100%' }}
              imgStyle={{ height: '100%' }}
            />
            <Caption text="Color palette — Sky, Navy, Orange & Yellow" />
          </div>
        </div>
      </motion.section>
      ...
      >
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '0 16px' : '0 36px' }}>
          <p style={{ ...EYEBROW, color: SKY }}>The concept</p>

          {isMobile ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              <p style={{ fontSize: 38, fontWeight: 500, color: '#ffffff', maxWidth: '80%', margin: 0, lineHeight: 1.3 }}>
                One letter.<br />Infinite direction.
              </p>
              <Img
                src={v('design-vaction-pattern.png')}
                alt="V.Action brand pattern"
                maxHeight="380px"
                wrapStyle={{ height: '380px' }}
                imgStyle={{ height: '100%' }}
              />
              <p style={{ fontSize: 28, lineHeight: 1.7, color: MUTED, margin: 0 }}>
                The "V" motif was extracted from the logotype and transformed into
                a repeating geometric pattern — peaks, valleys, movement. It works
                across apparel, digital surfaces and environmental applications.
              </p>
            </div>
          ) : (
      ...
      >
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '32px 16px' : '48px 36px', boxSizing: 'border-box' }}>
          <div style={{
            ...
            {[
              { num: '5',    label: 'Brand colors defined' },
              { num: '360°', label: 'Identity system coverage' },
              { num: '1',    label: 'Geometric motif, endless use' },
            ].map(card => (
              <div key={card.num} style={{ background: '#f0f0ee', padding: 24 }}>
                <p style={{ fontSize: 44, fontWeight: 500, color: BG, margin: '0 0 6px' }}>
                  {card.num}
                </p>
      ...
      >
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '32px 16px 160px' : '48px 36px 140px', boxSizing: 'border-box' }}>
          ...
          <div style={{
            ...
            <div>
              <Img
                src={v('design-vaction-bag.png')}
                alt="V.Action tote bag"
                maxHeight="480px"
                wrapStyle={{ height: '480px' }}
                imgStyle={{ height: '100%' }}
              />
              <Caption text="Tote bag — merch application" />
            </div>
            <div>
              <Img
                src={v('design-vaction-mug.png')}
                alt="V.Action mug"
                maxHeight="480px"
                wrapStyle={{ height: '480px' }}
                imgStyle={{ height: '100%' }}
              />
              <Caption text="Mug — lifestyle brand touchpoint" />
            </div>
          </div>
          <div>
            <Img
              src={v('design-moodboard.png')}
              alt="V.Action moodboard"
              maxHeight="480px"
              wrapStyle={{ height: '480px', width: '100%' }}
              imgStyle={{ height: '100%' }}
            />
            <Caption text="Moodboard — Canadian winter & adventure" />
          </div>
        </div>
      </motion.section>
      ...
      <footer style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {['Illustrator', 'Photoshop', 'Brand Strategy', 'Identity Design'].map(tool => (
              <span key={tool} style={{
