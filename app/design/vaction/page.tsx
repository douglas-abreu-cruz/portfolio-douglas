...
const p = (f: string) => `/images/vaction/${f}`
...
const STATS = [
  { num: '1',    label: 'Brand system' },
  { num: '2',    label: 'Color worlds' },
  { num: '100%', label: 'Concept to mockup' },
]

const TOOLS = ['Illustrator', 'Canva']

export default function VActionPage() {
    ...
    <div style={{ backgroundColor: BG, minHeight: '100vh', paddingBottom: isMobile ? '160px' : '140px' }}>
        ...
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '40px 16px' : '64px 36px 40px', display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? 32 : 0, alignItems: 'flex-start', overflow: 'visible' }}>
          <div style={{ width: isMobile ? '100%' : '60%', flexShrink: 0, paddingRight: isMobile ? 0 : 56, boxSizing: 'border-box' }}>
            ...
            <p style={{ fontSize: 28, lineHeight: 1.8, color: '#444', maxWidth: 520 }}>
              A brand for a winter tourism agency targeting young people and families who want adventure with comfort. Cool tones reflect winter, warm accents bring coziness and fire.
            </p>
          </div>
          <div style={{ width: isMobile ? '100%' : '40%', marginTop: isMobile ? 0 : -100, position: 'relative', zIndex: 2 }}>
            <Img src={p('design-vaction-hero.png')} alt="V.Action" maxHeight="60vh" imgStyle={{ boxShadow: '0 16px 48px rgba(0,0,0,0.2)' }} />
          </div>
        </div>
      </motion.section>
      ...
      <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={VP} transition={FV}
        ...
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '0 16px 40px' : '0 36px 64px', display: 'flex', flexDirection: 'column', gap: 32 }}>
          <Img src={p('design-vaction-moodboard.png')} alt="Moodboard" maxHeight="480px" wrapStyle={{ height: '480px', width: '100%' }} imgStyle={{ height: '100%' }} />
          <Img src={p('design-vaction-pattern.png')} alt="Pattern system" maxHeight="480px" wrapStyle={{ height: '480px', width: '100%' }} imgStyle={{ height: '100%' }} />
        </div>
      </motion.section>
        ...
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '0 16px' : '0 36px' }}>
          ...
          {isMobile ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              ...
              <Img src={p('design-vaction-hero.png')} alt="V.Action" maxHeight="380px" wrapStyle={{ height: '380px' }} imgStyle={{ height: '100%' }} />
              <p style={{ fontSize: 28, lineHeight: 1.7, color: MUTED, margin: 0 }}>
                The contrast between cold and warm became the brand visual pattern. Applied consistently across every touchpoint from logo to merchandise.
              </p>
            </div>
          ) : (
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 32 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <Img src={p('design-vaction-hero.png')} alt="V.Action" maxHeight="480px" wrapStyle={{ height: '480px' }} imgStyle={{ height: '100%', boxShadow: '0 10px 36px rgba(0,0,0,0.22)' }} />
              </div>
            </div>
          )}
        </div>
      ...
      <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={VP} transition={FV}
        ...
        <div style={{ maxWidth: 1440, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 24 : 32, marginBottom: isMobile ? 24 : 32 }}>
            <div>
              <Img src={p('design-vaction-bag.png')} alt="Tote bag" maxHeight="480px" wrapStyle={{ height: '480px' }} imgStyle={{ height: '100%' }} />
              <Caption text="Tote bag" />
            </div>
            <div>
              <Img src={p('design-vaction-mug.png')} alt="Merchandise" maxHeight="480px" wrapStyle={{ height: '480px' }} imgStyle={{ height: '100%' }} />
              <Caption text="Merchandise" />
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 24 : 32 }}>
            <div>
              <Img src={p('design-vaction-hero.png')} alt="Logo application" maxHeight="480px" wrapStyle={{ height: '480px' }} imgStyle={{ height: '100%' }} />
              <Caption text="Logo application" />
            </div>
            <div>
              <Img src={p('design-vaction-pattern.png')} alt="Pattern system" maxHeight="480px" wrapStyle={{ height: '480px' }} imgStyle={{ height: '100%' }} />
              <Caption text="Pattern system" />
            </div>
          </div>
        </div>
      </motion.section>
